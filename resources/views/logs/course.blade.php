...
<div id="body">
    <div class="max-w-7xl mx-auto">
        <div class="header-section">
            ...
            <a href="{{ url('uni-logs') }}" class="course-log-link">
                University Logs
            </a>

            <!-- Centered Heading -->
            <strong><h1><a href="{{ route('adminDashboard') }}">Programme Logs</a></h1></strong>
        ...
        $(document).ready(function () {
            ...
            $('#clear-logs').click(function () {
                if (confirm('Are you sure you want to clear all logs?')) {
                    $.ajax({
                        url: '/clear-course-logs', // Ensure this endpoint is correct
                        method: 'POST',
                        headers: {
            ...
            function fetchLogs() {
                $.ajax({
                    url: '/fetch-course-logs', // Updated endpoint
                    method: 'GET',
                    success: function (response) {
            ...
            function displayLogs(logs) {
                ...
                logs.split('\n').forEach(log => {
                    ...
                    let action = 'Other';
                    let rowClass = '';
                    if (log.includes('New Programme Added') || log.includes('New Programme Created')) {
                        action = 'Added';
                        rowClass = 'log-added';
                    } else if (log.includes('Programme Update')) {
                        action = 'Updated';
                        rowClass = 'log-updated';
                    } else if (log.includes('Programme Deleted')) {
                        action = 'Deleted';
                    ...
                    const keywords = [
                        'New Course Added:',
                        'Course Update - Before Changes',
                        'Course Update - After Changes',
                        'Course Deleted',
                        'New Course Created'
                    ];
        ...
        document.getElementById('downloadPdf').addEventListener('click', function () {
            ...
            doc.html(element, {
                callback: function (doc) {
                    // Save the PDF
                    doc.save('Programmes_Logs_Report.pdf');
